...
impl RepositoryValidator {
    ...
    pub fn validate(&self, repository_path: &Path) -> SharedResult<ValidationReport> {
    ...
    pub fn auto_repair(&self, repository_path: &Path) -> SharedResult<ValidationReport> {
    ...
    ) -> SharedResult<Option<RepositoryVersion>> {
        let metadata_path = path.join("metadata.yml");
        if !metadata_path.exists() {
            return Ok(None);
        }

        let content = fs::read_to_string(&metadata_path).map_err(|e| SharedError::Internal {
            message: format!("Failed to read metadata.yml: {}", e),
        })?;
    ...
    fn validate_credentials_directory(
        ...
        let credentials_dir = path.join("credentials");
        let entries = fs::read_dir(&credentials_dir).map_err(|e| SharedError::Internal {
            message: format!("Failed to read credentials directory: {}", e),
        })?;
    ...
    fn validate_types_directory(
        ...
        let types_dir = path.join("types");
        let entries = fs::read_dir(&types_dir).map_err(|e| SharedError::Internal {
            message: format!("Failed to read types directory: {}", e),
        })?;
    ...
    fn calculate_directory_size(&self, dir_path: &Path) -> SharedResult<u64> {
    ...
    fn migrate_legacy_format(&self, repository_path: &Path) -> SharedResult<bool> {
        ...
        if metadata_path.exists() {
            ...
            let mut metadata: serde_yaml::Value =
                serde_yaml::from_str(&content).map_err(|e| SharedError::InvalidFormat {
                    message: format!("Failed to parse metadata for migration: {}", e),
                })?;
